  ...
  <div>
    <a-menu v-model="current" mode="horizontal">
      ...
      <a-sub-menu>
        ...
        >增加HTTP头字段</span
        >
          <a-menu-item key="setting:5" @click="headerFieldObject.switch = true,headerFieldObject.name='',headerFieldObject.key=''">自定义头字段</a-menu-item>
          <a-divider />
    ...
    <div >
      <a-modal class="modal"
               title="HTTP 头字段"
               v-model="headerFieldObject.switch"
               @ok="sendHeaderField"
               @cancel="cancelHeaderField"
               okText="确认"
               cancelText="取消">
        <div class="name">
          <div class="title">名称</div>
          <a-input v-model="headerFieldObject.name" placeholder="头字段名称" />
        </div>
       <div class="key">
         <div class="title">值</div>
         <a-input v-model="headerFieldObject.key" placeholder="头字段值" />
       </div>
      </a-modal>
    </div>
  </div>
    ...
    methods: {
      ...
      sendHeaderField(){
        if(!window.localStorage){
          ...
          // 定义对象
          let obj = {id:Date.now(), name:this.headerFieldObject.name, key:this.headerFieldObject.key, switch:false}
          let arr =[]
          // 从localStorage中获取所用地址
        ...
        this.headerFieldObject.name = ''
        this.headerFieldObject.key = ''
